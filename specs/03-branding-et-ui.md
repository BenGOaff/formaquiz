# FormaQuiz : branding et UI (répliquer les codes de Tiquiz)

> L'interface FormaQuiz doit reprendre les codes visuels de Tiquiz (couleurs, typographie, composants, mise en page) en version FormaQuiz. Même famille visuelle, identité propre.
>
...
FormaQuiz est le petit frère de Tiquiz. Un élève qui connaît Tiquiz doit se sentir chez lui. On reprend le même système de design (shadcn/ui + Tailwind + variables CSS HSL + police Inter), on l'habille aux couleurs FormaQuiz, et on garde la même qualité de finition SaaS.

Concrètement : copier la structure du design system de Tiquiz dans le nouveau projet (le même `tailwind.config.ts`, le même fichier de variables CSS, les mêmes composants de base shadcn/ui), puis ajuster le nom, le logo et, si Béné le souhaite, une couleur d'accent FormaQuiz.
...
- **Animations** : `quiz-step-in` (les questions montent et apparaissent en douceur), `shimmer` pour les skeletons. À réutiliser pour le flux de quiz FormaQuiz, ça donne le même feeling guidé.
...
## Identité propre FormaQuiz

- **Nom** : FormaQuiz. Logo à fournir par Béné (même esprit que le logo Tiquiz).
- **Couleur d'accent** : par défaut on garde l'indigo Tiquiz pour la cohérence de marque. Si Béné veut différencier FormaQuiz, prévoir une seule variable de couleur d'accent à changer (ne pas disperser les couleurs en dur dans le code).
...
- Le flux de quiz FormaQuiz reprend l'animation `quiz-step-in` de Tiquiz pour l'effet guidé question par question.
...
1. Copier le `tailwind.config.ts` et le fichier de variables CSS de Tiquiz dans le projet FormaQuiz.
2. Installer le même socle shadcn/ui (voir `components.json` de Tiquiz).
3. Brancher la police Inter.
4. Centraliser la couleur d'accent dans une seule variable, pour pouvoir différencier FormaQuiz facilement si besoin.
