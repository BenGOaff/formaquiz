...
function block(b: DocBlock): string {
  ...
  if (b.kind === "steps") {
    return `<div class="steps">${b.items
      .map((i) => `<div class="step"><span class="badge">${esc(i.label)}</span><div>${inline(i.text)}</div></div>`)
      .join("")}</div>`;
  }
  return `<div class="sub"><p class="subtitle">${esc(b.title)}</p>${b.blocks.map(block).join("")}</div>`;
}
...
<body>
  ...
  ${doc.lead.length > 0 ? `<div class="lead">${doc.lead.map(block).join("")}</div>` : ""}
  ${sections}
