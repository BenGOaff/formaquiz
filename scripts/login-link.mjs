...
// l'email dans le flux normal (app/api/auth/magic-link/route.ts). La
...
if (!url || !key) {
  console.error(
    "Impossible de lire NEXT_PUBLIC_SUPABASE_URL et SUPABASE_SERVICE_ROLE_KEY.\n" +
      "Lance le script depuis le dossier de l'Atelier, à côté de son fichier .env.",
  );
  process.exit(1);
}
...
if (!res.ok || !link) {
  ...
  console.error("Vérifie que ce compte existe bien sur l'Atelier.");
  process.exit(1);
}
