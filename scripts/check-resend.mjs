...
// L'Atelier écrit depuis `hello@tiquiz.fr` aussi (Béné, 30 août).
const REPLI = "hello@tiquiz.fr";

const cle = readVar("RESEND_API_KEY");
const brut =
  readVar("FORMAQUIZ_EMAIL_FROM") || readVar("QUIZING_EMAIL_FROM") || REPLI;
...
if (!readVar("FORMAQUIZ_EMAIL_FROM") && !readVar("QUIZING_EMAIL_FROM")) {
  console.log("  (FORMAQUIZ_EMAIL_FROM absente : aucun email ne partira, voir lib/email/resend.ts)");
}
