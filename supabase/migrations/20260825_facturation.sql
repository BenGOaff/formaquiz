...
--
-- Le module a été construit pour Tiquiz le 24 août, et l'Atelier avait
-- exactement le même trou : son bon de commande PayPal encaissait sans
-- qu'aucune facture n'existe. `lib/checkout/paypalOwner.ts` n'appelle
-- aucun point d'entrée de facturation, et la commande qu'on crée ne
-- porte ni adresse ni numéro de TVA.
--
-- **LA DIFFÉRENCE DE FOND AVEC TIQUIZ : l'Atelier vend un ACHAT
-- UNIQUE.** Une vente, une facture. Pas d'échéance, donc le déclencheur
-- est la CAPTURE (`PAYMENT.CAPTURE.COMPLETED`) et pas une vente v1.
--
-- La série est `AQ-<année>`, jamais `TQ` : les deux apps ont leur propre
-- compteur, dans leur propre base. Avec le même préfixe, elles
-- émettraient chacune un `TQ-2026-0001` pour deux ventes différentes.
--
