...
test("le webhook ecoute le remboursement, et l'abonne a Stripe", () => {
  ...
  assert.ok(
    route.includes("await revokeAccessByEmail("),
    "la revocation a disparu de la route",
  );
  assert.ok(
    route.includes("refundGoodbyeEmail("),
    "l'email d'au revoir a disparu : l'acheteur ne recevrait que celui de Stripe",
  );
});

test("l'acces est coupe sur l'adresse de la VENTE, pas sur celle de la carte", () => {
...
test("l'email d'au revoir ne contient aucun tiret cadratin", () => {
  ...
  const src = fs.readFileSync(path.join(process.cwd(), "lib/email/templates.ts"), "utf8");
  const debut = src.indexOf("export function refundGoodbyeEmail");
  assert.ok(debut > 0, "refundGoodbyeEmail a disparu");
