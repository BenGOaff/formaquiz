...
// (Béné, 11 septembre 2026). Jumeau du test de Tiquiz : l'Atelier poste
// vers le même registre, par le même chemin.
//
...
function corps(nomFonction: string): string {
  const debut = OWNER.indexOf(`async function ${nomFonction}(`);
  assert.ok(debut >= 0, nomFonction);
  const suite = OWNER.indexOf("\nasync function ", debut + 1);
  const suiteExport = OWNER.indexOf("\nexport ", debut + 1);
  const fins = [suite, suiteExport].filter((i) => i !== -1);
  return OWNER.slice(debut, fins.length ? Math.min(...fins) : undefined);
}

test("l'ATTRIBUTION met en attente sur un échec, et libère sur un succès", () => {
  const c = corps("attribuerChezTipote");
...
test("l'ANNULATION passe par le MÊME filet : sinon la commission mûrit et part au lot", () => {
  const c = corps("annulerCommissionChezTipote");
