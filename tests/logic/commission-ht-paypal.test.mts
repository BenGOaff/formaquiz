...
//
// UNE VENTE PAYPAL PAIE L'AFFILIÉ SUR LE HT, COMME UNE VENTE CARTE.
//
...
// ment.
//
// L'Atelier paie 70 % : sur une vente à 47 € TTC, ça fait 32,90 € au
// lieu de 27,42 €. Ce sont EXACTEMENT les deux chiffres du drame du
// 19 août, où l'app annonçait 32,90 € et payait 27,42 €. On avait
// corrigé ce que l'app ANNONCE ; le chemin PayPal, lui, versait encore
// les 32,90 €.
//
// -- LE MOYEN QU'ELLE CHERCHAIT EXISTAIT DÉJÀ -------------------------
//
// Depuis le 25 août, c'est NOUS qui émettons la facture d'une vente
// PayPal : `construireFacture` résout le régime (pays, numéro de TVA)
// et décompose le TTC. Le webhook lit désormais la taxe de CETTE
// facture là. Montant facturé et montant commissionné sortent du même
// calcul, par construction.
//
// Jumeau de `tests/logic/commission-ht-paypal.test.mts` chez Tiquiz.
...
function factureDe(pays: string | null, numeroTva: string | null, totalCents = 4700) {
...
describe("La TVA d'une vente PayPal se calcule chez nous", () => {
  test("UN ACHETEUR FRANÇAIS : 47,00 EUR TTC -> commission sur 39,17 EUR", () => {
    const f = factureDe("FR", null);
    const taxe = taxePaypalCents(f, 4700);
    assert.equal(taxe, 783, "la TVA francaise sur 47,00 EUR TTC");
    const base = commissionBaseCents(4700, taxe);
    assert.equal(base, 3917);
    // 70 % du HT = 27,42 EUR, et non 32,90 EUR.
    assert.equal(Math.round(base * 0.7), 2742);
  });
  ...
  test("UN PROFESSIONNEL EN AUTOLIQUIDATION paie une commission sur TOUT", () => {
    ...
    // retirer 20 % "par principe" sous-paierait l'affilie.
    const f = factureDe("BE", "BE0123456789");
    const taxe = taxePaypalCents(f, 4700);
    assert.equal(taxe, 0);
    assert.equal(commissionBaseCents(4700, taxe), 4700);
  });

  test("UN ACHETEUR HORS UE : pas de TVA, donc rien à retirer", () => {
    assert.equal(taxePaypalCents(factureDe("CA", null), 4700), 0);
  });
  ...
  test("LA TAXE COMMISSIONNÉE EST CELLE DE LA FACTURE, au centime", () => {
    for (const pays of ["FR", "DE", "ES", "IT", "PT", "CA"]) {
      ...
      assert.equal(taxePaypalCents(f, 4700), Math.abs(f.tvaCents), `pays ${pays}`);
      assert.equal(commissionBaseCents(4700, taxePaypalCents(f, 4700)), Math.abs(f.htCents));
    }
  });
});
...
describe("Sans facture, on ne devine pas : on retient, et on crie", () => {
  test("PAS DE FACTURE -> taux du pays du vendeur, jamais zéro", () => {
    ...
    assert.equal(taxePaypalCents(null, 4700), 783);
    assert.ok(taxeEstUnRepli(null, 4700), "le repli doit se signaler");
  });

  test("une taxe LÉGITIMEMENT à zéro n'est PAS un repli", () => {
    assert.equal(taxeEstUnRepli({ totalCents: 4700, tvaCents: 0 }, 4700), false);
    assert.equal(taxePaypalCents({ totalCents: 4700, tvaCents: 0 }, 4700), 0);
  });

  test("une taxe absurde tombe dans le repli", () => {
    assert.ok(taxeEstUnRepli({ totalCents: 4700, tvaCents: 99999 }, 4700));
    assert.equal(taxePaypalCents({ totalCents: 4700, tvaCents: 99999 }, 4700), 783);
  });
});
...
describe("Le webhook PayPal ne repasse plus zéro", () => {
  ...
  test("LE MONTANT ENCAISSÉ passe devant celui de la commande", () => {
    // La capture porte ce qui a vraiment ete paye (une remise
    // comprise) : decomposer une TVA calculee sur un total et la
    // retirer d'un AUTRE donnerait une base fausse qui a l'air juste.
    assert.match(WEBHOOK, /encaissement\?\.totalCents \?\? commande\?\.amountTotalCents/);
  });
