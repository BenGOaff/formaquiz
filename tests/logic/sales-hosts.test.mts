...
//
// Chantier du 20 aout : atelierduquiz.fr sert la page de vente et le bon de
...
test("le domaine de vente ouvre la page, avec ou sans cle", () => {
  assert.equal(isSalesOpen(null, "atelierduquiz.fr", ENV), true);
  assert.equal(isSalesOpen(null, "www.atelierduquiz.fr", ENV), true);
  // Le port ne change rien : un reverse proxy peut le laisser passer.
  assert.equal(isSalesOpen(null, "atelierduquiz.fr:443", ENV), true);
  // La casse non plus.
  assert.equal(isSalesOpen(null, "ATELIERDUQUIZ.FR", ENV), true);
});
...
test("l'app reste FERMEE sans la cle", () => {
  ...
  assert.equal(isSalesOpen(null, "quizing.tipote.com", ENV), false);
  assert.equal(isSalesOpen("mauvaise-cle", "quizing.tipote.com", ENV), false);
  assert.equal(isSalesOpen(CLE, "quizing.tipote.com", ENV), true);
...
test("sans SALES_PREVIEW_TOKEN, seule la porte du domaine reste", () => {
  ...
  assert.equal(isSalesOpen(CLE, "quizing.tipote.com", {}), false);
  assert.equal(isSalesOpen(null, "atelierduquiz.fr", {}), true);
});

test("le domaine dit QUELLE page il sert", () => {
  assert.equal(salesSlugForHost("atelierduquiz.fr"), "atelier-du-quiz");
  assert.equal(salesSlugForHost("quizing.tipote.com"), null);
  assert.equal(isPublicSalesHost("atelierduquiz.fr"), true);
  assert.equal(isPublicSalesHost("quizing.tipote.com"), false);
});
...
test("on ramene l'acheteur LA OU IL A ACHETE (20 aout 2026)", () => {
  ...
  // avoir paye.
  assert.equal(
    checkoutReturnBase("https://atelierduquiz.fr", "https://quizing.tipote.com"),
    "https://atelierduquiz.fr",
  );
  assert.equal(
    checkoutReturnBase("https://www.atelierduquiz.fr", "https://quizing.tipote.com"),
    "https://www.atelierduquiz.fr",
  );
});
...
test("un Host falsifie ne detourne PAS le retour de paiement", () => {
  ...
  for (const origine of [
    ...
    "https://atelierduquiz.fr.mechant.example",
    "http://localhost:3000",
    "pas-une-url",
    "",
    null,
    ...
    assert.equal(
      checkoutReturnBase(origine, "https://quizing.tipote.com"),
...
test("depuis l'app, on garde le domaine canonique", () => {
  ...
  assert.equal(
    checkoutReturnBase("https://quizing.tipote.com", "https://quizing.tipote.com"),
    "https://quizing.tipote.com",
  );
});
