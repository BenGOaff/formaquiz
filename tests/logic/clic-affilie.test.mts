...
// reste, `atelierduquiz.fr/?ref=jocelyne`. Il posait déjà le cookie d'un an,
...
test("une page vue avec un code affilié compte un clic", () => {
  ...
  assert.equal(clicASignaler({ ref: "jocelyne", pathname: "/commande/atelier", accept: PAGE }), true);
});
...
test("une image ou un appel d'API ne sont PAS des clics", () => {
  ...
  assert.equal(clicASignaler({ ref: "jocelyne", pathname: "/api/commande", accept: PAGE }), false);
